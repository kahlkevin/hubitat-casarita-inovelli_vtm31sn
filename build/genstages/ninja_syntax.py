...
Patched by Kevin Kahl to add inputs_per_line support for build rules
"""
...
class Writer(object):
    ...
    def build(
        ...
        inputs_per_line: bool = False,
    ) -> List[str]:
        outputs = as_list(outputs)
        out_outputs = [escape_path(x) for x in outputs]
        explicit_inputs = [escape_path(x) for x in as_list(inputs)]
        implicit_inputs = []
        order_only_inputs = []
        all_inputs = explicit_inputs[:]

        if implicit:
            implicit_inputs = [escape_path(x) for x in as_list(implicit)]
            all_inputs.append('|')
            all_inputs.extend(implicit_inputs)
        if order_only:
            order_only_inputs = [escape_path(x) for x in as_list(order_only)]
            all_inputs.append('||')
        ...
        if inputs_per_line:
            self._line_with_input_groups(
                'build %s: %s' % (' '.join(out_outputs), rule),
                explicit_inputs,
                implicit_inputs,
                order_only_inputs,
            )
    ...
    def _line_with_continuation_tokens(
        self,
        first_line: str,
        tokens: List[str],
        indent: int = 0,
    ) -> None:
        """Write tokens one per continuation line."""
        if not tokens:
            self._line(first_line, indent)
            return

        leading_space = '  ' * indent
        continuation_space = '  ' * (indent + 2)
        self.output.write(leading_space + first_line + ' $\n')

        for token in tokens[:-1]:
            self.output.write(continuation_space + token + ' $\n')
        self.output.write(continuation_space + tokens[-1] + '\n')

    def _line_with_input_groups(
        self,
        first_line: str,
        ...
        """Write build inputs one per line, preserving Ninja input groups."""
        if not explicit_inputs and not implicit_inputs and not order_only_inputs:
            self._line(first_line, indent)
            return
        ...
        def write_input_group(tokens: List[str], separator: str = '') -> None:
            for token in tokens[:-1]:
                self.output.write(continuation_space + token + ' $\n')
            if tokens:
                self.output.write(continuation_space + tokens[-1] + separator)

        if explicit_inputs:
            self.output.write(leading_space + first_line + ' $\n')
            write_input_group(
                explicit_inputs,
                ' |$\n' if implicit_inputs else
                ' ||$\n' if order_only_inputs else '\n',
            )
        elif implicit_inputs:
            self.output.write(leading_space + first_line + ' |$\n')
        else:
            self.output.write(leading_space + first_line + ' ||$\n')
        ...
        if order_only_inputs:
            write_input_group(order_only_inputs, '\n')
